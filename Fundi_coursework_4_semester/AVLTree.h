...
private:
    ...
    void innerInsert(AVLTreeNode<T>** pTreeNode, const T& inputValue)// вставка ключа inputValue в дерево с корнем root
    {
        ...
        switch (comparisonResult)
        {
            case -1:
                if ((*pTreeNode)->left == nullptr)
                    ...
                    innerInsert(reinterpret_cast<AVLTreeNode<T>**>(&((*pTreeNode)->left)), (inputValue));
                break;
            case 1:
                if ((*pTreeNode)->right == nullptr)
                    (*pTreeNode)->right = new AVLTreeNode<T>(inputValue);
                else
                    innerInsert(reinterpret_cast<AVLTreeNode<T>**>(&((*pTreeNode)->right)), (inputValue));
                break;
            case 0:
    ...
    void innerDelete(AVLTreeNode<T>** pTreeNode, const T& inputValue)
    {
        AVLTreeNode<T>* tmp;
        if (*pTreeNode == nullptr)
            throw std::invalid_argument("The tree does not exist!");
        int comparisonResult = this->strategy_->compare(inputValue, (*pTreeNode)->value);
        switch (comparisonResult)
        {
            case -1:
                innerDelete(reinterpret_cast<AVLTreeNode<T>**>(&((*pTreeNode)->left)), (inputValue));
                break;
            case 1:
                innerDelete(reinterpret_cast<AVLTreeNode<T>**>(&((*pTreeNode)->right)), (inputValue));
                break;
            case 0:
                ...
                else
                {
                    ...
                    innerDelete(reinterpret_cast<AVLTreeNode<T>**>(&((*pTreeNode)->right)), (tmp->value));
                }
                break;
            default:
                break;
    ...
    AVLTreeNode<T>* getNodeByValue(AVLTreeNode<T>* pTreeNode, const T& inputValue)
    {
...
public:
    ...
    void addNode(const T& data) override
    {
        innerInsert(reinterpret_cast<AVLTreeNode<T>**>(&this->root), (data));
    }
    TreeNode<T>* searchByValue(const T& data) override
    {
        return getNodeByValue(reinterpret_cast<AVLTreeNode<T>*>(this->root), (data));
    }
    void printTree()
    {
    ...
    {
        innerDelete(reinterpret_cast<AVLTreeNode<T>**>(&this->root), (data));
    }
    bool empty()
    {
